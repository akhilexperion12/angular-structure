...
export class BaseHttpService {
    ...
    private _handleError(error: Response | any) {
        ...
        if (error.status === 400) {
            ...
            errorResponse.errorMessage = body.message;
            if (body.data) {
                errorResponse.errorResponseData = body.data;
            }
            ...
            if (this._authService.isLoggedIn()) {
                ...
                // this._broadcastService.broadcastLogout(false);
                // this._broadcastService.broadcastSuccessLogin(false);
                this._broadcastService.broadcastAlert('error', body.message);
    ...
    public findById(url: object, queryParams: object, disableLoader?: boolean): Observable<any> {
        this._requestInterceptor(disableLoader);
    ...
    public uploadFile(url: object, file: any, body: any, disableLoader?: boolean): Observable<any> {
        this._requestInterceptor(disableLoader);
        const headers = new Headers();
        this._setRequestHeader(headers); headers.delete('Content-Type');
        ...
        return this._http.post(this._setUrl(url), formData, options).map((response: any) => {
            this._responseInterceptor(disableLoader); return this._responseHandler(response);
            })
            .catch((error: any) => {
                this._responseInterceptor(disableLoader);
                return this._handleError(error);
