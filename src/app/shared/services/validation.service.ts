...
export class ValidationService {
    ...
    static forgotPasswordValidator(control) {
        // RFC 2822 compliant regex
        if (control.value) {
            // tslint:disable-next-line:max-line-length
            if (control.value.match(/(?:[a-z0-9!#$%&'*+/=?^_`{|}~-])+(?:\.[a-z0-9!#$%&'*+/=?^_`{|}~-]+)*@(?:[a-z0-9](?:[a-z0-9-]*[a-z0-9])?\.)+[a-z0-9](?:[a-z0-9-]*[a-z0-9])?/)) {
                return null;
            } else if (control.value.match(/^[\+]?[(]?[0-9]{3}[)]?[-\s\.]?[0-9]{3}[-\s\.]?[0-9]{4,6}$/)
                && control.value.length > 9 && control.value.length < 16) {
                return null;
            } else {
    ...
    static termsAndConditionsValidator(control) {
        if (control.value) {
            return null;
        } else {
            return { 'termsViolation': true };
        }
    }
}
